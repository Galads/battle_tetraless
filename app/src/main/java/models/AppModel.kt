...
class AppModel {
    ...
    var currentState = Status.AWAITING_START.name

    private var field: Array<ByteArray> = arrayBytesModel(  //game field
            RegionConstants.ROW_COUNT.value,
            RegionConstants.COLUMN_COUNT.value
    )
    ...
    private fun generalNextBlock() {
        currentBlock = Block.createBlock() //generate next block
    }
    ...
    private fun validTranslation(position: Point, shape: Array<ByteArray>):
        ...
        return if (position.y < 0 || position.x < 0) {
            false
        } else if (position.y + shape.size > RegionConstants.ROW_COUNT.value) {
            false
        } else if (position.x + shape[0].size > RegionConstants.COLUMN_COUNT.value) {
            false
        } else {
    ...
    private fun resetField(ephemeralCellOnly: Boolean = true) {
        for (i in 0 until RegionConstants.ROW_COUNT.value) {
            (0 until RegionConstants.COLUMN_COUNT.value)
    ...
    private fun assessField() {
        for (i in 0 until field.size) { //field.size = 20
            var emptyCells = 0
            for (j in 0 until field[i].size) {
